...
class desk {
public:
	cell* deskstate[64],**allstates[100][64];
	...
	int askPiece(int player)
	{
		...
		cin >> piece;
		if(int(piece[0])<97 || int(piece[0])>122 || int(piece[1])<48 || int(piece[1])>57)
		{
...
int main()
{
	...
	while (check==2)
	{
		...
		*newgame->allstates[counter]=newgame->deskstate;
		counter += 1;
	}
